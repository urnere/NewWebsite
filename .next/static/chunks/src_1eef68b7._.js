(globalThis.TURBOPACK = globalThis.TURBOPACK || []).push(["static/chunks/src_1eef68b7._.js", {
...
const addToRemoveQueue = (toastId)=>{
    if (toastTimeouts.has(toastId)) {
        clearTimeout(toastTimeouts.get(toastId)); // Clear existing timeout if present
    }
...
// Ensure that `dispatch` function is defined before being used in reducer
let dispatch;
const reducer = (state, action)=>{
    switch(action.type){
        case "ADD_TOAST":
            // Automatically dismiss older toasts when a new one arrives
        ...
        case "DISMISS_TOAST":
            {
                ...
                if (toastId) {
                    ...
                    state.toasts.forEach((toast)=>{
                        if (toast.open) addToRemoveQueue(toast.id); // Only queue removal for open toasts
                    });
                }
                return {
                    ...state,
...
// Define dispatch function before useToast hook
dispatch = (action)=>{
